...
class TriangleMultiplication(Module):
    ...
    def __call__(self, x: ttnn.Tensor) -> ttnn.Tensor:
        ...
        ttnn.DumpDeviceProfiler(mesh_device.get_devices()[1])
...
class TriangleAttention(Module):
    ...
    def __call__(self, x: ttnn.Tensor) -> ttnn.Tensor:
        ...
        triangle_bias = ttnn.linear(
            x_i, self.bias_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        triangle_bias = ttnn.reshape(triangle_bias, (1, *triangle_bias.shape))
        triangle_bias = ttnn.permute(triangle_bias, (3, 0, 1, 2))
        q = ttnn.linear(
            x_i, self.q_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        k = ttnn.linear(
            x_i, self.k_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        v = ttnn.linear(
            x_i, self.v_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        ...
        a_m = ttnn.matmul(q, k, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.DRAM_MEMORY_CONFIG,)
        
        ttnn.deallocate(q)
        ttnn.deallocate(k)

        a = ttnn.multiply(a_m, self.head_dim**-0.5)
        #ttnn.deallocate(a_m)
        ...
        g = ttnn.linear(
            x_i, self.g_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
...
class AttentionPairBias(Module):
    ...
    def __call__(self, s: ttnn.Tensor, z: ttnn.Tensor) -> ttnn.Tensor:
        ...
        k = ttnn.linear(
            s, self.k_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        v = ttnn.linear(
            s, self.v_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        q = ttnn.permute(q, (2, 0, 1))
        k = ttnn.permute(k, (2, 0, 1))
        v = ttnn.permute(v, (2, 0, 1))
        ...
        z = ttnn.linear(
            z, self.z_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        ...
        o = ttnn.permute(o, (1, 2, 0))
        g = ttnn.linear(
            s, self.g_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        g = ttnn.sigmoid_accurate(g)
        o = ttnn.multiply(o, g)
        x = ttnn.linear(
...
class Transition(Module):
    ...
    def __call__(self, x: ttnn.Tensor) -> ttnn.Tensor:
        ...
        x_1 = ttnn.linear(
            x_norm, self.fc1_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )
        
        x_1 = ttnn.silu(x_1)
        x_2 = ttnn.linear(
            x_norm, self.fc2_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )
        x = ttnn.multiply(x_1, x_2)
        x = ttnn.linear(
            x, self.fc3_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        return x
...
class PairformerLayer(Module):
    ...
    def __call__(
        ...
        z = ttnn.add(
            z,
            self.triangle_multiplication_start(z), memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        z = ttnn.add(
            z,
            self.triangle_multiplication_end(z), memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        z = ttnn.add(
            z,
            self.triangle_attention_start(z), memory_config=ttnn.L1_MEMORY_CONFIG,
        )
        z = ttnn.add(
            z,
            self.triangle_attention_end(z), memory_config=ttnn.L1_MEMORY_CONFIG,
        )
...
class ConditionedTransitionBlock(Module):
    ...
    def __call__(self, a: ttnn.Tensor, s: ttnn.Tensor) -> ttnn.Tensor:
        ...
        a_b = ttnn.linear(
            a, self.a_to_b_weight, compute_kernel_config=self.compute_kernel_config, memory_config=ttnn.L1_MEMORY_CONFIG,
        )
